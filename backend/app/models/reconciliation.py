"""
Core reconciliation models: BankAccount, Reconciliation, Transactions, Matches, Anomalies.
"""
...
class BankAccount(Base):
    ...
    reconciliations = relationship("Reconciliation", back_populates="bank_account", cascade="all, delete-orphan")
...
class Reconciliation(Base):
    ...
    bank_transactions = relationship("BankTransaction", back_populates="reconciliation", cascade="all, delete-orphan")
    ledger_transactions = relationship("LedgerTransaction", back_populates="reconciliation", cascade="all, delete-orphan")
...
class Match(Base):
    """Records a match between bank and ledger transaction(s)."""
    __tablename__ = "matches"
    
    id = Column(Integer, primary_key=True, index=True)
    reconciliation_id = Column(Integer, ForeignKey("reconciliations.id", ondelete="CASCADE"), nullable=False)
    
    # Can be 1-to-1, 1-to-many, or many-to-1
    # Store as comma-separated IDs for simplicity (or use junction table for production)
    bank_transaction_ids = Column(String(500), nullable=False)  # e.g., "1,2,3"
    ledger_transaction_ids = Column(String(500), nullable=False)
    
    match_type = Column(String(50), default="exact")  # exact, fuzzy, manual
    confidence_score = Column(Float, default=1.0)  # 0-1 score for fuzzy matches
